...
logfile_path_staff = r'' + os.path.abspath(os.path.dirname(os.path.dirname(__file__))) + config['log'][
                         'logfile_path'] + logfile_name
print(logfile_path_staff)

# 打到终端的日志级别和格式
terminal_level = config['log']['terminal_level']
terminal_formatter = config['log']['terminal_formatter']
# 打到文件的日志级别和格式
file_level = config['log']['file_level']
file_formatter = config['log']['file_formatter']
# 既打到终端又打到文件的日志级别
all_level = config['log']['all_level']
# 日志文件存储字节数
max_bytes = int(config['log']['max_bytes'])
# 日志文件轮转数
backup_count = int(config['log']['backup_count'])
# 日志文件编码方式
