...
class Assembler:
    ...
    def __init__(self):
        ...
        # redis 工具装配
        if config['project']["redis_enable"].upper() == "Y":
            # 装配 redis 连接池工具
            self.assemble_redis()
        elif config['project']["redis_enable"].upper() == "N":
            self.redis_pool_tool = None
        else:
            self.redis_pool_tool = None
            raise RuntimeError("配置文件中配置 redis 是否启用字段有误！请修改！")
        # mysql 工具装配
        if config['project']["mysql_enable"].upper() == "Y":
            # 装配 mysql 工具
            self.assemble_mysql()
        elif config['project']["mysql_enable"].upper() == "N":
    ...
    def assemble_driver(self):
        # 若是谷歌驱动
        if config['project']["driver"].lower() == "chrome":
            # chrome option
            ...
            # 使用远端浏览器
                ...
                url = "http://" + config['project']["remote_ip"] + ":" + config['project'][
                    "remote_port"] + "/wd/hub"
                self.driver = RemoteWebDriver(command_executor=url, options=chrome_options)

        # 若是火狐驱动
        elif config['project']["driver"].lower() == "firefox":
            # firefox option
        ...
        # 若是 IE 驱动
            ...
            executable_path = os.path.abspath(os.path.dirname(__file__))[
                              :os.path.abspath(os.path.dirname(__file__)).find("test-framework") + len(
                                  "test-framework")] + "/ui-test" + config['driver']["ie_driver_path"]
            self.driver = webdriver.Ie(executable_path=executable_path, ie_options=ie_options)

        # 若是 Edge 驱动
        elif config['project']["driver"].lower() == "edge":
            executable_path = os.path.abspath(os.path.dirname(__file__))[
                              :os.path.abspath(os.path.dirname(__file__)).find("test-framework") + len(
                                  "test-framework")] + "/ui-test" + config['driver'][
                                  "edge_driver_path"]
        ...
        # 若是欧朋驱动
        elif config['project']["driver"].lower() == "opera":
            executable_path = os.path.abspath(os.path.dirname(__file__))[
                              :os.path.abspath(os.path.dirname(__file__)).find("test-framework") + len(
                                  "test-framework")] + "/ui-test" + config['driver'][
                                  "opera_driver_path"]
        ...
        # 若是 Safari 驱动
        elif config['project']["driver"].lower() == "safari":
            executable_path = os.path.abspath(os.path.dirname(__file__))[
                              :os.path.abspath(os.path.dirname(__file__)).find("test-framework") + len(
                                  "test-framework")] + "/ui-test" + config['driver'][
                                  "safari_driver_path"]
