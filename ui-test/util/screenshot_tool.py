...
class ScreenshotTool:
    ...
    def save_img(self, driver, img_name):
        ...
        img_path = os.path.abspath(os.path.dirname(__file__))[
                   :os.path.abspath(os.path.dirname(__file__)).find("test-framework") + len(
                       "test-framework")] + "/ui-test" + config['screenshot']["shotfile_path"]
        # 获取新的图片名
        new_img_name = self.get_img_name(img_name)
        # 根据配置文件的配置，截图智能存放
        driver.get_screenshot_as_file(
            ("{}/{}" + config['screenshot']["shot_format"]).format(img_path, new_img_name))
    ...
    def get_img_name(self, img_name):
        ...
        if config['screenshot']["cover_allowed"].upper() == "Y":
            # 返回截图名字
            return img_name
        # 若不支持覆盖同名图片
        elif config['screenshot']["cover_allowed"].upper() == "N":
            # 判断图片路径是否存在，若存在
            if os.path.exists(img_path + img_name + config['screenshot']["shot_format"]):
                # 如果名字不以 ")" 结尾
